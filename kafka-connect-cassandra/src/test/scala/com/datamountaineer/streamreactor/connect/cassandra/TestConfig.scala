...
trait TestConfig extends StrictLogging with MockitoSugar {
  ...
  def getCassandraConfigSinkProps2Tables = {
    Map(
      ...
      CassandraConfigConstants.USERNAME -> USERNAME,
      CassandraConfigConstants.PASSWD -> PASSWD,
      CassandraConfigConstants.SINK_KCQL -> s"INSERT INTO $TABLE6 SELECT id, int_field1, double_field1,timestamp_field1 FROM $TOPIC67; INSERT INTO $TABLE7 SELECT id, int_field2, double_field2,timestamp_field2 FROM $TOPIC67",
  ...
  def getCassandraConfigSinkPropsNestedFieldsTables = {
    Map(
      ...
      CassandraConfigConstants.USERNAME -> USERNAME,
      CassandraConfigConstants.PASSWD -> PASSWD,
      CassandraConfigConstants.SINK_KCQL -> s"INSERT INTO $TABLE8 SELECT id, inner1.int_field, inner2.* FROM $TOPIC8",
      CassandraConfigConstants.ERROR_POLICY -> ErrorPolicyEnum.NOOP.toString
    ).asJava
  }
  ...
  def getCassandraConfigSinkPropsNestedFieldsStructTables = {
    Map(
      ...
      CassandraConfigConstants.USERNAME -> USERNAME,
      CassandraConfigConstants.PASSWD -> PASSWD,
      CassandraConfigConstants.SINK_KCQL -> s"INSERT INTO $TABLE9 SELECT id, inner1.int_field, inner2.* FROM $TOPIC9",
      CassandraConfigConstants.ERROR_POLICY -> ErrorPolicyEnum.NOOP.toString
    ).asJava
  }
